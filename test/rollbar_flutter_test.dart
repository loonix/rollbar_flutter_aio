...
void main() {
  ...
  test('Initialize platform component when running application', () async {
    final config = Config(accessToken: 'SomeAccessToken', package: 'some_package_name', includePlatformLogs: true, handleUncaughtErrors: true, sandbox: AsyncSandbox.new, sender: (_) => sender);
    ...
    await RollbarFlutter.run(config, () {
      ...
      expect(init.arguments['handleUncaughtErrors'], equals(config.handleUncaughtErrors));
      expect(init.arguments['includePlatformLogs'], equals(config.includePlatformLogs));
    });
  });
  ...
  test('Add platform_payload if PlatformException is enriched', () async {
    ...
    await RollbarFlutter.run(config, () async {
      ...
      expect(frames[0]['class_name'], equals('com.rollbar.flutter.example.MainActivity'));
      ...
      expect(frames[1]['class_name'], equals('com.rollbar.flutter.example.MainActivity'));
    });
  });
}
