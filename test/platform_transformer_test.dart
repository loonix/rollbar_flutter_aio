...
import 'package:rollbar_flutter_aio/common/rollbar_common.dart' hide isNotNull;
import 'package:rollbar_flutter_aio/rollbar_dart/rollbar.dart';
import 'package:rollbar_flutter_aio/src/platform_transformer.dart';
  ...
  group('PlatformTransformer tests', () {
    ...
    group('Platform transformer (appendToChain: true)', () {
      ...
      test('Enrich trace chain on PlatformException', () async {
        ...
        final body = Body(telemetry: const [], report: _Trace.from(exception, frames));
      ...
      test('Create chain on PlatformException with trace', () async {
        ...
        const frames = [Frame(filename: filename, method: 'onTheDartSide', line: 3)];

        final trace = _Trace.from(exception, frames);
        final original = _Data.from(body: Body(telemetry: const [], report: trace));
        final transformed = await transformer.transform(
          original,
          event: ErrorEvent(exception, StackTrace.empty),
        );
        ...
        expect(rethrownTrace.frames.elementAt(0).method, equals('processError'));
        expect(rethrownTrace.frames.elementAt(1).method, equals('catchAndThrow'));
    ...
    group('Platform transformer (appendToChain: false)', () {
      ...
      test('Attach platform payload on PlatformException with chain', () async {
        ...
        final original = _Data.from(body: Body(telemetry: const [], report: platformTrace));
        final transformed = await transformer.transform(
          original,
          event: ErrorEvent(exception, StackTrace.empty),
        );
        ...
        expect(transformed.platformPayload?['data']['notifier']['name'], equals('rollbar-java'));
      ...
      test('Attach platform payload on PlatformException with trace', () async {
        ...
        const frames = [Frame(filename: filename, method: 'attachedFailureChain', line: 3)];
        ...
        final chain = transformed.platformPayload?['data']['body']['trace_chain'];
