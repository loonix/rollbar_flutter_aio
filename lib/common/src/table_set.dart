...
class TableSet<E extends Persistable<UUID>> with SetMixin<E> implements Set<E> {
  ...
  @override
  Iterator<E> get iterator => database.select('SELECT * FROM $tableName').map(deserialize).iterator;
  ...
  E? record({required UUID id}) => database.select('SELECT ${keys.join(', ')} FROM $tableName WHERE id = ?', [id.toBytes()]).trySingle.map(deserialize);
  ...
  bool contains(Object? element) {
    ...
    final result = database.select('SELECT EXISTS(SELECT 1 FROM $tableName WHERE id = ?)', [element.id.toBytes()]);
