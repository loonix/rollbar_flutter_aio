...
class PlatformTransformer implements Transformer {
  ...
  Future<Data> transform(Data data, {required Event event}) async {
    ...
    if (defaultTargetPlatform.isAndroid && error is PlatformException && data.body.report.isTrace) {
      data = error.traces
          .map((extraTraces) => data.body.report.traces.attach(extraTraces, append))
          .map((allTraces) => data.copyWith(body: data.body.copyWith(report: Traces(allTraces)), platformPayload: error.payload))
          .or(data);
    }
...
extension _PlatformException on PlatformException {
  ...
  Iterable<Trace>? get traces => (payload?['data']['body'] as JsonMap?).map(Report.fromMap)?.traces;

  JsonMap? get payload => message.mapIf((rawPayload) => rawPayload.startsWith(_prefix), (rawPayload) => jsonDecode(rawPayload.substring(_prefix.length)));
}
...
extension _Message on Iterable<Trace> {
  ...
  Iterable<Trace> attach(Iterable<Trace> extraTraces, bool append) =>
      ...
      (extraTraces.tryFirst?.exception.message).map((message) => 'PlatformException(error, "$message")').map(replaceExceptionMessages).map(append ? extraTraces.followedBy : identity).or(this);
  ...
  Iterable<Trace> replaceExceptionMessages(String message) =>
      mapWhere((trace) => trace.exception.message.containsJsonPayload, (trace) => trace.copyWith(exception: trace.exception.copyWith(message: message)));
}

extension _String on String {
  bool get containsJsonPayload => startsWith('PlatformException') && contains(_prefix);
}
