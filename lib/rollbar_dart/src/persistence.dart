...
mixin Persistence<Record extends Persistable<UUID>> implements Configurable {
  ...
  late final TableSet<Record> records = TableSet(database: () {
    try {
      _database ??= config.persistenceLifetime > Duration.zero ? sqlite3.open('${config.persistencePath}/rollbar.db') : sqlite3.openInMemory();
      ...
      log('Exception opening sqlite3 database to persist for ${config.persistenceLifetime}',
          time: DateTime.now(), level: Level.warning.value, name: runtimeType.toString(), error: error, stackTrace: stackTrace);
    } finally {
  ...
  bool didExpire(Record record) => record.timestamp < DateTime.now().toUtc() - config.persistenceLifetime;
}
