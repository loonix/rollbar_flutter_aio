...
class PersistentHttpSender extends HttpSender with Persistence<PayloadRecord> {
  ...
  void log(final Object o, [final StackTrace? stackTrace]) {
    if (o is http.Response) {
      developer.log('\'${o.statusCode} ${o.reasonPhrase}\' sending payload to \'$uri\'',
          name: 'Rollbar.${runtimeType.toString()}', time: DateTime.now(), level: Level.error.value, error: o.result.failure, stackTrace: StackTrace.current);
    } else if (o is http.ClientException) {
      developer.log('${o.message} while trying to reach \'${o.uri}\'', name: 'Rollbar.${runtimeType.toString()}', time: DateTime.now(), level: Level.critical.value, error: o, stackTrace: stackTrace);
    }
  }
}
